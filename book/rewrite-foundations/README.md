...
版本：**0.5，2026-09-19**。已逐页通读本地 PDF 的 Part II，覆盖正文、公式、例子、表格及占位内容；对部分符号和疑似问题另行查看页面图像。这个状态表示已经完成内容梳理，不表示独立验证了每一条公式、算法和所引文献。正文已完成 L01–L03，其余小节按提纲继续编写。

开始阅读：[L01 · 为什么二值逻辑的常识需要重新检查](lessons/01-two-valued-logic.md)。从订单条件出发，解释真值、逻辑等价及其适用范围，并完整推导吸收律。

继续阅读：[L02 · NULL 改变了哪些操作](lessons/02-null-and-comparisons.md)。区分普通值相等、SQL 相等和点等号，解释 NULL 传播及分组中的比较语义。

最新一节：[L03 · 三个真值与两种解释上下文](lessons/03-three-valued-logic.md)。从三值真值表进入 WHERE/CHECK，区分原始真值与接受行为，并解释排中律为何失效。
...
下一节为 **L04：否定为什么会交换 UNKNOWN 的解释**，继续第 5 章。遇到 tuple、函数、集合等基础记号时就地解释；后续在第 7 章统一建立精确定义，不先用另一篇论文重排整本书。
