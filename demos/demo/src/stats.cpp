...
void StatsCatalog::AddRelationStats(const std::string &alias, RelationStats stats) {
  relations_.emplace(alias, stats);
}

void StatsCatalog::AddSelectivity(const ColumnRef &left, const ColumnRef &right, double selectivity) {
  if (selectivity <= 0.0) {
    throw std::runtime_error("selectivity must be positive");
  }
  const auto key = PredicateKey(left, right);
  const auto found = selectivities_.find(key);
  if (found != selectivities_.end() && found->second != selectivity) {
    throw std::runtime_error("conflicting selectivity for predicate: " +
                             left.ToString() + " = " + right.ToString());
  }
  selectivities_[key] = selectivity;
}
...
double StatsCatalog::LookupSelectivity(const ColumnRef &left, const ColumnRef &right) const {
  ...
  if (found == selectivities_.end()) {
    throw std::runtime_error("missing selectivity for predicate: " +
                             left.ToString() + " = " + right.ToString());
  }
  return found->second;
}
...
// --- JSON parsing (kept for optional backward compatibility) ---
...
private:
  JsonValue ParseValue() {
    ...
    if (ConsumeLiteral("true")) return {JsonValue::Kind::Bool, true, 0, "", nullptr, nullptr};
    if (ConsumeLiteral("false")) return {JsonValue::Kind::Bool, false, 0, "", nullptr, nullptr};
  ...
  std::string ParseString() {
    ...
    while (pos_ < input_.size()) {
      ...
      if (c == '"') return result;
      if (c == '\\') {
        if (pos_ >= input_.size()) throw std::runtime_error("unterminated JSON escape");
...
StatsCatalog StatsCatalog::FromJson(const std::string &json) {
  ...
  for (const auto &[alias, relation_value] : relations.object_value->fields) {
    ...
    catalog.AddRelationStats(alias, {NumberAt(relation_value, "rows"),
  ...
  for (const auto &entry : selectivities.array_value->values) {
    ...
    catalog.AddSelectivity(ParseColumnRef(StringAt(entry, "left")),
                           ParseColumnRef(StringAt(entry, "right")),
                           NumberAt(entry, "selectivity"));
  }
...
StatsCatalog StatsCatalog::FromFile(const std::string &path) {
  ...
  if (!input) throw std::runtime_error("failed to open stats file: " + path);
